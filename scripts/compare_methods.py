...
def main():
    ...
    all_samplers = ["ddim", "plms", "k_lms", "k_euler", "k_euler_ancestral", "k_heun", "k_dpm_2", "k_dpm_2_ancestral",]
    fontname = "DejaVuSerif-Bold"
    font = ImageFont.truetype(fontname, size=40)
    ...
    all_grids = []
    for sampling_mode in all_samplers:
        torch.cuda.empty_cache()
        is_k = False
        print(f"sampling_mode: {sampling_mode}")
        if sampling_mode == "plms":
            ...
            is_k = True
            if sampling_mode == "k_lms":
                sampler = K.sampling.sample_lms
            elif sampling_mode == "k_euler":
                sampler = K.sampling.sample_euler
            elif sampling_mode == "k_euler_ancestral":
                sampler = K.sampling.sample_euler_ancestral
            elif sampling_mode == "k_heun":
                sampler = K.sampling.sample_heun
            elif sampling_mode == "k_dpm_2":
                sampler = K.sampling.sample_dpm_2
            elif sampling_mode == "k_dpm_2_ancestral":
        ...
        grid_count = 0 # len(os.listdir(outpath)) - 1
            ...
            with precision_scope("cuda"):
                with model.ema_scope():
                    ...
                    if not opt.skip_grid:
                        ...
                        img_grid = Image.fromarray(grid.astype(np.uint8))
                        img_grid = put_watermark(img_grid, wm_encoder)
                        # img.save(os.path.join(outpath, f'grid-{grid_count:04}.png'))
                        # grid_count += 1
                        # put caption
                        ImageDraw.Draw(img_grid).text((0, 0), sampling_mode, (255, 255, 255), font=font)
                        all_grids.append(img_grid)

                    toc = time.time()
        del sampler
    w, h = all_grids[0].size
    all_img = Image.new("RGB", (w, len(all_grids) * h))
    for idx, image in enumerate(all_grids):
        all_img.paste(image, (0, idx * h))
    all_img.save("out.png")
